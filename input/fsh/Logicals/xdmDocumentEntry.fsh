...
* availabilityStatus 1..1 code "Cette métadonnée représente la pertinence de la version de la fiche d'un document." "**Availability Status**"
...
* documentAvailability 0..1 string "Cette métadonnée représente l’accessibilité du document en indiquant si celui-ci est accessible en ligne ou non"
...
* version 0..1 integer "Cette métadonnée représente le numéro de version de la fiche d’un document.  La valeur de la métadonnée version est égale à 1 pour la première version de la fiche."
...
* class 1..1 XDMCode "class représente la classe du document (compte rendu, imagerie médicale, traitement, certificat, etc.)."
* class from https://mos.esante.gouv.fr/NOS/JDV_J06-XdsClassCode-CISIS/FHIR/JDV-J06-XdsClassCode-CISIS (preferred)
  * code ^short = "ClassCode de la classe du document."
  * code ^definition = """Les valeurs possibles doivent être un code provenant du jeu de valeurs mis à disposition par le projet (exemple : JDV_J57_ClassCode_DMP). En l'absence de spécifications complémentaires, le JDV_J06_XdsClassCode_CISIS peut être utilisé."""
  * displayName ^short = "Libellé associé au classCode."
  * codingScheme ^short = "OID du système de codage associé au code classCode."
* confidentiality 1..4 XDMCode "Métadonnée contenant les informations définissant le niveau de confidentialité d'un document déposé dans l'entrepôt. Dans le cadre de la mise en œuvre du masquage et de la non-visibilité, ces métadonnées sont utilisées pour rendre inaccessible un document à l'utilisateur."
...
* eventCodeList 0..* XDMCode "Cette métadonnée contient les codes, libellés et codes système représentant :  -un évènement documenté (acte, traitement, diagnostic, etc…), -une modalité d'acquisition (contexte imagerie), -une région anatomique (contexte imagerie)."
* format 1..1 XDMCode "Métadonnée contenant les informations définissant le format du document. Il précise les spécifications techniques auxquelles il est conforme"
* format from https://mos.esante.gouv.fr/NOS/JDV_J10-XdsFormatCode-CISIS/FHIR/JDV-J10-XdsFormatCode-CISIS (preferred)
  * code ^short = "code identifiant le format du document et les spécifications auxquelles il est conforme. Fixé à urn:asipSante:modelesHorsProfils:2011 (Document non référencé IHE ou CI-SIS) pour les documents au format propriétaire." 
* healthcareFacilityTypeCode 1..1 XDMCode "Secteur d'activité lié à la prise en charge de la personne, en lien avec le document produit." "Healthcare Facility Type Code**"
...
* practiceSetting 1..1 XDMCode "Contexte de l’acte qui a engendré la création du document."
...
* type 1..1 XDMCode "Métadonnée représentant le type du document." "**Type Code**"
...
* referenceIdList 0..* XDMReferenceId "Cette métadonnée contient une liste d'un ou plusieurs identifiant(s) d'objet(s) associé(s) au document. Liste d'éléments de type CX du standard HL7 v2.5."
