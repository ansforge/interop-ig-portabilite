...
**Sortie** : Archive ZIP conforme à la structure définie dans [Vue d'ensemble technique](spec-technique-vue-ensemble.html) contenant :
...
2. Lire `MANIFEST.JSON` pour connaître le nombre de patients et le statut de l'export
3. Parcourir les SUBSETs et importer les documents CDA structurés
4. Importer les données du dossier `SPECIFIC/` en utilisant le `mapping.json`
...
2. **Lot 2 (complémentaire)** : Données historiques — selon calendrier convenu. `exportStatus: COMPLETE`, `batchNumber: 2`.
