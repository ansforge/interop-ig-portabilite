...
Il doit :
- Être capable de lire et d'importer les formats définis dans l'[Annexe 2 du référentiel](spec-technique-vue-ensemble.html)
...
2. Pour chaque `DocumentEntry`, récupérer le fichier via le champ `URI` (chemin relatif dans le SUBSET)
3. Importer les documents CDA R2 (VSM, documents Ségur) selon les volets CI-SIS correspondants
...
2. Importer les notes de consultation, l'agenda, les paramètres vitaux selon la documentation du dictionnaire de données
3. En cas d'impossibilité d'import automatique, présenter les données brutes au professionnel de santé pour saisie manuelle
...
| Donnée | Format à importer | Référence |
|---|---|---|
